...
DistanceSpacing: 1                          # Défini la distance de déplacement de la prochaine note selon le temps 
                                            # écoulé depuis la dernière note. Pour l'utilisation de l'éditeur
BeatDivisor: 4                              # C'est le diviseur de temps dans l'editeur. Pour faire simple, 4 battements
                                            # du metronome valent une mesure. Pour les musiciens qui ont besoin de ces 
                                            # outils pour mieux utiliser l'éditeur
GridSize: 16                                # 4, 8, 16, 32, c'est la distance entre chaques points de la grille
                                            # d'édition. Il sert à l'utilisation de l'editeur
...
[Difficulty] # Voila ce qui nous intéresse
HPDrainRate:7                               # Quantité de vie que l'on perd dans le temps et quand on rate une note
CircleSize:4                                # Taille des notes
OverallDifficulty:9                         # Marge d'erreur. Par exemple si la note est à la position 100ms, si on la
                                            # touche entre 50ms et 150ms on récupère quand même la note (en gros, c'est
                                            # la fourchette de validation)
ApproachRate:9                              # Temps que les notes restes à l'écran. Plus c'est haut, moins de temps les
                                            # notes restent à l'écran
...
# Sert pour l'éditeur puisqu'il permet de mettre à jour le status du bpm, du volume de la musique, des pack de 
# hitsounds, etc mais une foit les cercles positionnés, la map ne change pas selon ces paramètres (pour sa partie 
# "gameplay")
...
# A: Inconnu
# B: Inconnu
# HitSound: Défini les HitSound de la note sous différentes forme, dans la suite 1:3:0:0 le 1 dit qu'il s'agit du son
#           principal "Normal" auquel on ajoute grace au 3 le son secondaire "Drum". Les deux à la fin je nai pas trouvé
#           à quoi ils servent
# ListePointsBezier: Il s'agit d'une liste qui contient tous les points (en dehors du point initial donnée par 
#           x,y,Offset) pour produire la courbe de Bezier du slider.
#                    Voici les 2 formats de cette liste:
#                       - L|x:y Le L semble vouloir dire "Linear" mais semble apparaitre que pour 1 seul point dans la
#                         liste. Avec une liste à 2 points dont j'ai forcé la linéarité, c'est quand même passé au
                          second format
...
# C: Inconnu
# HitSoundOptionnel:  Même format que pour HitSound sauf que l'on ajoute une liste avant sous la forme: 0:0|...|0:0 qui
#                     correspondent respectivement aux points dans ListPointBezier
'
    ...
    with open(file, encoding='utf-8') as map_data:
        ...
        for line in map_data:
            ...
            if "[Difficulty]" in section:
                content = line.split(":")
                if len(content) > 1:
                    data[content[0]] = content[1][:len(content[1]) - 1]
